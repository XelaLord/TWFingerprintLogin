...
def InitScanner():
    ...
    if init[0] == 'E':
        ...
        ser.write(bytes('0;','utf-8'))
        print("[ERROR] Wrong script loaded onto Arduino, please upload \"Enroll.ino\" to the Arduino")
...
while True:
    ...
    while ser.in_waiting > 0:
        ser.read()
    ser.write(bytes(str(available[0]),'utf-8'))
    ...
    if flag:
        ...
        try:
            sheet.append_row([available[0],name] + ['' for i in range(sheet.col_count-2)])
            available.pop(0)
            ser.write(bytes("1;",'utf-8'))
